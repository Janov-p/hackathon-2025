...
<template>
  <div class="min-h-screen bg-cm-gray">
    ...
    <div class="print:hidden sticky top-0 z-50 bg-white shadow-md">
      <div class="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
        <button
          @click="goBack"
          class="flex items-center gap-2 text-cm-dark hover:text-cm-red transition-colors"
        >
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            ...
            class="flex items-center gap-2 px-5 py-2.5 bg-white border-2 border-cm-red text-cm-red rounded-lg font-semibold hover:bg-cm-red hover:text-white transition-all"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            @click="printPdf"
            class="flex items-center gap-2 px-5 py-2.5 bg-cm-red text-white rounded-lg font-semibold hover:bg-cm-dark transition-all"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="max-w-5xl mx-auto p-6 print:p-0 print:max-w-none">
      <div class="bg-white rounded-2xl shadow-xl overflow-hidden print:rounded-none print:shadow-none">
        <!-- En-tête -->
        <div class="bg-cm-red p-8 text-white print:p-6">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-3xl font-bold">Plan Média</h1>
              <p class="text-white/80 mt-1">{{ formData.nom || 'Campagne' }}</p>
            </div>
            <div class="text-right">
              <div class="text-sm text-white/70">Généré le</div>
              <div class="font-semibold">{{ new Date().toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' }) }}</div>
            </div>
          </div>
        </div>

        <!-- Informations campagne -->
        <div class="p-8 border-b border-gray-200 print:p-6">
          <h2 class="text-lg font-bold text-cm-dark mb-4 flex items-center gap-2">
            <svg class="w-5 h-5 text-cm-red" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <div class="font-semibold text-gray-800">{{ formData.nom || '-' }}</div>
            </div>
            <div>
              <div class="text-xs text-gray-500 uppercase tracking-wider">Secteur</div>
              <div class="font-semibold text-gray-800">{{ formData.secteur || '-' }}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div class="font-semibold text-gray-800">{{ formData.zones?.join(', ') || '-' }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="p-8 border-b border-gray-200 print:p-6">
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div class="bg-gray-50 p-4 rounded-xl border-l-4 border-cm-red">
              <div class="text-xs text-gray-500 uppercase tracking-wider">Audience Cumulée</div>
              <div class="text-2xl font-bold text-gray-800">{{ formatNumber(kpiResults.audienceCumulee) }}</div>
            ...
            <div class="bg-gray-50 p-4 rounded-xl border-l-4 border-cm-red">
              ...
              <div class="text-2xl font-bold text-gray-800">{{ kpiResults.tauxCouverture }}%</div>
            </div>
            <div class="bg-gray-50 p-4 rounded-xl border-l-4 border-cm-red">
              <div class="text-xs text-gray-500 uppercase tracking-wider">Fréquence Moyenne</div>
              <div class="text-2xl font-bold text-gray-800">{{ kpiResults.frequenceMoyenne }}x</div>
            </div>
            <div class="bg-gray-50 p-4 rounded-xl border-l-4 border-cm-red">
              <div class="text-xs text-gray-500 uppercase tracking-wider">GRP</div>
              <div class="text-2xl font-bold text-gray-800">{{ kpiResults.grp }}</div>
            </div>
            <div class="bg-gray-50 p-4 rounded-xl border-l-4 border-cm-red">
              <div class="text-xs text-gray-500 uppercase tracking-wider">Impressions</div>
              <div class="text-2xl font-bold text-gray-800">{{ formatNumber(kpiResults.impressions) }}</div>
            </div>
            <div class="bg-gray-50 p-4 rounded-xl border-l-4 border-cm-red">
              <div class="text-xs text-gray-500 uppercase tracking-wider">Clics Estimés</div>
              <div class="text-2xl font-bold text-gray-800">{{ formatNumber(kpiResults.clicsEstimes) }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="p-8 border-b border-gray-200 print:p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div
              ...
              :key="index"
              class="bg-gray-50 p-4 rounded-xl border-l-4 border-cm-red"
            >
              <div class="text-xs font-bold text-cm-red uppercase tracking-wider">{{ support.type }}</div>
              <div class="font-semibold text-gray-800 mt-1">{{ support.nom }}</div>
              <div class="text-sm text-gray-500 mt-1">{{ support.justification }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="p-8 print:p-6">
          ...
          <div class="bg-cm-red rounded-xl p-6 text-white mb-6">
            <!-- Ligne Brut HT -->
            <div class="flex justify-between items-center pb-3 border-b border-white/20">
              <div class="text-sm text-white/70 uppercase tracking-wider">Brut HT</div>
              <div class="text-xl font-semibold">{{ formatCurrency(brutHT) }}</div>
            </div>
            
            <!-- Ligne Remise (si active et visible) -->
            <div v-if="remise.active && remise.visible" class="flex justify-between items-center py-3 border-b border-white/20">
              <div class="text-sm text-white/70 uppercase tracking-wider">
                Remise {{ remise.type === 'pourcentage' ? `(${remise.valeur}%)` : '' }}
              </div>
              <div class="text-xl font-semibold text-green-300">- {{ formatCurrency(montantRemise) }}</div>
            </div>
            
            <!-- Ligne Net HT -->
            <div class="flex justify-between items-center py-3 border-b border-white/20">
              <div class="text-sm text-white/70 uppercase tracking-wider">Net HT</div>
              <div class="text-2xl font-bold">{{ formatCurrency(netHT) }}</div>
            </div>
            
            <!-- Ligne TVA -->
            <div class="flex justify-between items-center py-3 border-b border-white/20">
              <div class="text-sm text-white/70 uppercase tracking-wider">TVA (20%)</div>
              <div class="text-xl font-semibold">{{ formatCurrency(tva) }}</div>
            </div>
            
            <!-- Ligne Total TTC -->
            <div class="flex justify-between items-center pt-3">
              <div class="text-sm text-white/70 uppercase tracking-wider">Total TTC</div>
              <div class="text-3xl font-bold">{{ formatCurrency(totalTTC) }}</div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div
              ...
              :key="key"
              class="bg-gray-50 p-4 rounded-xl text-center"
            >
              <div class="text-xs text-gray-500 uppercase tracking-wider capitalize">{{ key }}</div>
              <div class="text-xl font-bold text-gray-800 mt-1">{{ formatCurrency(value) }}</div>
            </div>
          </div>
        </div>

        <!-- Footer -->
        <div class="bg-gray-100 px-8 py-4 text-center text-sm text-gray-500 print:bg-gray-50">
